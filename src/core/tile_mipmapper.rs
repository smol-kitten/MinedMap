...
impl TileCollector for TileMipmapper<'_> {
	...
	fn prepare(&self, level: usize) -> Result<()> {
		...
		if self.config.biome_layer {
			fs::create_dir_all(&self.config.tile_dir(TileKind::Biomemap, level))?;
		}
		...
		if self.config.contour_layer {
			fs::create_dir_all(&self.config.tile_dir(TileKind::Contourmap, level))?;
		}
	...
	fn collect_one(
		...
		let biome_stat = if self.config.biome_layer {
			self.render_mipmap::<image::Rgba<u8>>(TileKind::Biomemap, level, coords, prev)?
		} else {
			MipmapStat {
				total: 0,
				processed: 0,
		...
		let cave_stat = if self.config.cave_layer {
			self.render_mipmap::<image::Rgba<u8>>(TileKind::Cavemap, level, coords, prev)?
		} else {
			MipmapStat {
				total: 0,
				processed: 0,
		...
		let contour_stat = if self.config.contour_layer {
			self.render_mipmap::<image::Rgba<u8>>(TileKind::Contourmap, level, coords, prev)?
		} else {
			MipmapStat {
				total: 0,
				processed: 0,
